...
import (
	...
	"github.com/charan-kumar-137/fortpay/constant"
	"github.com/charan-kumar-137/fortpay/dto"
	"github.com/charan-kumar-137/fortpay/model"
...
func setupRouter(dbConfig *badger.DB) *mux.Router {
...
func PaymentsAuthorization(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal
	var authorizationRequestDto dto.AuthorizationRequest
	...
	if len(businessAccountId) > 0 {
		...
		json.NewEncoder(w).Encode(dto.CreatePaymentResponse(authorizationRequestDto.OrderId, "3", "Invalid Merchant"))
		return
	}
	if businessAccount.Id != businessAccountId {
		json.NewEncoder(w).Encode(dto.CreatePaymentResponse(authorizationRequestDto.OrderId, "15", "No Such Issuer"))
		return
	}
	...
	if len(personalAccountId) > 0 {
		...
		json.NewEncoder(w).Encode(dto.CreatePaymentResponse(authorizationRequestDto.OrderId, "12", "Invalid Card Number"))
		return
	}
	if personalAccount.Id != personalAccountId {
		json.NewEncoder(w).Encode(dto.CreatePaymentResponse(authorizationRequestDto.OrderId, "56", "No Card Record"))
		return
	}
	...
	if personalAccount.CardNumber != authorizationRequestDto.CardNumber ||
		...
		json.NewEncoder(w).Encode(dto.CreatePaymentResponse(payment.Id, "5", "Do Not Honour"))
		return
	}
	...
	if personalAccount.Available < authorizationRequestDto.Amount {
		...
		businessAccount.Statement = append(businessAccount.Statement, payment.Id)
		saveAccount(db, businessAccount)
		json.NewEncoder(w).Encode(dto.CreatePaymentResponse(payment.Id, "51", "Insufficient Funds"))
		return
	}
	...
	saveAccount(db, personalAccount)
	json.NewEncoder(w).Encode(dto.CreatePaymentResponse(payment.Id, "0", "Approved"))
...
func PaymentsCapture(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal
	var successiveRequestDto dto.SuccessiveRequest
	...
	if len(businessAccountId) > 0 {
		...
		json.NewEncoder(w).Encode(dto.CreatePaymentResponse(referenceId, "3", "Invalid Merchant"))
		return
	}
	if businessAccount.Id != businessAccountId {
		json.NewEncoder(w).Encode(dto.CreatePaymentResponse(referenceId, "15", "No Such Issuer"))
		return
	}
	...
	if len(referenceId) <= 0 {
		json.NewEncoder(w).Encode(dto.CreatePaymentResponse(referenceId, "12", "Invalid Transaction	"))
		return
	}
	...
	if referencedPayment.Operation == constant.AUTHORIZATION && referencedPayment.Status == "0" {
		if referencedPayment.CurrentAmount < successiveRequestDto.Amount {
			...
			saveAccount(db, businessAccount)
			json.NewEncoder(w).Encode(dto.CreatePaymentResponse(successivePayment.Id,
				"13",
				"Invalid Amount"))
			return
		} else {
	...
	json.NewEncoder(w).Encode(dto.CreatePaymentResponse(successivePayment.Id, "12", "Invalid Transaction"))
}
...
func AccountsDeposit(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal
	var accountRequestDto dto.AccountRequest
	...
	// Validate Account
	var accountResponseDto = dto.CreateAccountResponse(accountRequestDto.Id, "14", "Invalid Card Number")
	if len(accountRequestDto.Id) > 0 {
		...
		if len(account.Id) > 0 {
			...
			saveAccount(db, account)
			accountResponseDto = dto.CreateAccountResponse(accountRequestDto.Id, "0", "Approved")
		}
	}
